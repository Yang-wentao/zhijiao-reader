...
> **v1.1.0 —— 新增「知交云」订阅模式。** 不想自己申请 API key 的用户，填一个激活码即可使用（模型与额度由云端提供）；开源自建用户继续免费自带 key，功能完全一致。
>
...
| | 知交云（订阅版） | 自带 API key（免费） |
|---|---|---|
| 需要做什么 | 填一个激活码 | 自己申请 API key 并填入 |
| 模型与额度 | 云端提供，每月固定额度 | 你自己的账户，按量付费 |
| 适合谁 | 不想折腾 API 的读者 | 已有 key / 想完全自控的用户 |
| 支持的服务方 | 知交云 | DeepSeek、SJTU API、OpenAI、Local Codex、任意 OpenAI-compatible 接口 |

知交云的网关代码同样开源（见 [cloud/](cloud/)），你也可以用它自建一套给自己的课题组用。
...
- 右栏字号、行距独立调节并持久化
- 知交云订阅（激活码即用，顶栏显示本月剩余额度）或自带 API key
...
| **v1.1.0** | **知交云订阅模式：激活码即用、顶栏余额显示、设置页按「订阅 / 自带 key / 高级」分组** |
...
- **有知交云激活码**：服务提供方选「知交云」→ 粘贴激活码 → 「测试连接」看到余额 → 保存，完成。
