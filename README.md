...
> **v1.1 —— 新增「知交订阅」。** 不想自己申请 API key 的用户，填一个激活码即可使用，内置 DeepSeek v4-flash 与每月额度；开源自建用户继续免费自带 key，功能完全一致。
>
...
- 右栏字号、行距独立调节并持久化
- 知交订阅（激活码即用，内置 DeepSeek v4-flash）或自带 API key
...
| v1.1.0 | 知交订阅模式：激活码即用、设置页按「订阅 / 自带 key / 高级」分组 |
...
- **有知交订阅激活码**：服务提供方选「知交订阅」→ 粘贴激活码 → 「测试连接」看到余额 → 保存，完成。
