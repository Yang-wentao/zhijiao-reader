...
> **v1.1 —— 新增「知交订阅」。** 不想自己申请 API key 的用户，填一个订阅码即可使用，内置 DeepSeek v4-flash 与每月额度；开源自建用户继续免费自带 key，功能完全一致。
>
> v1.0.0 是本项目的稳定版基线：核心循环（**选中 → 两秒内看懂 → 继续读**）完整、可靠，经过日常真实使用检验。本项目由本人主导、与 AI 结对完成（Codex 与 Claude 先后承担主力开发）。
...
前后端都跑在你自己的电脑上；API key 只存本地，永不进 git。设置里一键切换服务方，保存前可先「测试连接」。客户端断开时后端会立即停止拉取模型输出——不为看不到的内容付费。

**两种使用方式，同一个应用：**

| | 知交订阅（订阅版） | 自带 API key（免费） |
|---|---|---|
| 需要做什么 | 填一个订阅码 | 自己申请 API key 并填入 |
| 模型与额度 | 云端提供，每月固定额度 | 你自己的账户，按量付费 |
| 适合谁 | 不想折腾 API 的读者 | 已有 key / 想完全自控的用户 |
| 支持的服务方 | 知交订阅 | DeepSeek、SJTU API、OpenAI、Local Codex、任意 OpenAI-compatible 接口 |

知交订阅的网关代码同样开源（见 [cloud/](cloud/)），你也可以用它自建一套给自己的课题组用。
...
- 右栏字号、行距独立调节并持久化
- 知交订阅（订阅码即用，内置 DeepSeek v4-flash）或自带 API key
- 多种 AI 服务方切换 + 连接测试 + 模型/思考力度选择
...
| v1.1.0 | 知交订阅模式：订阅码即用、设置页按「订阅 / 自带 key / 高级」分组 |
| v1.1.1 | 公式渲染三处修复（行内 `\tag`、撇号+上标、游离编号）；顶栏显示所用模型 |
| **v1.1.2** | **网页版上线（zhijiao-reader.com/app）；健康看门狗；每码限流与来源记录；窄栏工具栏单行显示** |
...
有知交订阅订阅码的话，打开 <https://zhijiao-reader.com/app/> 就能在浏览器里直接用：填入订阅码、拖入 PDF、选中即翻译，PDF 依旧只在你的设备上。网页版的划线批注仅当次会话有效、不写回 PDF 文件，也不支持 Obsidian 笔记与自带 API key——需要这些请用下面的桌面版。
...
- **有知交订阅订阅码**：服务提供方选「知交订阅」→ 粘贴订阅码 → 「测试连接」看到余额 → 保存，完成。
- **自带 API key**：选 DeepSeek / SJTU API / OpenAI / 自定义 → 填 API key → 「测试连接」通过 → 保存。
