...
- `cloud` (知交订阅) is the hosted subscription path and the **default/recommended provider**. It does not call a model API: it forwards to the gateway in `cloud/` (this repo, deployed on the maintainer's machine) with an activation code, and re-parses the gateway's SSE stream back into text. No API key, no model choice, no prompt building on the app side — the gateway owns all of that. `/api/cloud/balance` proxies the code's quota for the header chip.
- `deepseek`, `openai`, `sjtu`, `custom` are OpenAI-compatible HTTP clients (the bring-your-own-key path). **SJTU is a first-class provider**, not a "custom" variant — it has its own UI options and model list in both frontend and backend.
...
- Prompts live in **three places**: `server/prompts.ts` (OpenAI-compatible), `server/providers/codexPrompts.ts` (Codex CLI), and `cloud/prompts.mjs` (the gateway — a deliberate copy of `server/prompts.ts`; both files carry a SYNC NOTE). When changing prompts, update the cloud copy too, keep math-formula delimiters stable, and keep "term explanation" as its own section.
